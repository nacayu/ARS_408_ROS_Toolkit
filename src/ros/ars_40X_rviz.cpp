...
void ContinentalRadarRViz::clusters_callback(ars_40X::ClusterList cluster_list) {//这里将订阅到的点云信息进行marker格式转化并进一步显示出来
  ...
  for (auto cluster : cluster_list.clusters) {
    ...
    switch (cluster.prob_of_exist) {
      //choose to visualize depends on its probabily
      case INVALID: {
        marker.color.r = 0.0f;
        marker.color.g = 0.0f;
        marker.color.b = 0.0f;
    ...
    //to configurate the point scale and the time appears on the rviz window
    marker.scale.x = 0.1;
    marker.scale.y = 0.1;
    marker.color.a = 1.0;
    marker.lifetime.fromSec(0.5);//marker生存时间
...
void ContinentalRadarRViz::objects_callback(ars_40X::ObjectList object_list) {
  ...
  for (auto object : object_list.objects) {//traverse every point
    visualization_msgs::Marker marker;
