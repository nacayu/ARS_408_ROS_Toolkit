...
RadarCfgROS::RadarCfgROS(ros::NodeHandle &nh, ARS_40X_CAN *ars_40X_can) :
  ...
  //new added
  set_filter_min_lifetime_service_ = 
      nh.advertiseService("set_filter_min_lifetime", &RadarCfgROS::set_filter_min_lifetime, this);
  set_filter_max_lifetime_service_ = 
    nh.advertiseService("set_filter_max_lifetime", &RadarCfgROS::set_filter_max_lifetime, this);
  }
...
//new added
bool RadarCfgROS::set_filter_min_lifetime(
    RadarFilter::Request &req,
    RadarFilter::Response & /*res*/) {
  if (!radar_filter_cfg_->set_filter_min_lifetime(static_cast<uint64_t>(req.FilterLifetime_min))) {//修改filter的raw_data值
    return false;
  }
...
bool RadarCfgROS::set_filter_max_lifetime(
    RadarFilter::Request &req,
    RadarFilter::Response & /*res*/) {
  if (!radar_filter_cfg_->set_filter_max_lifetime(static_cast<uint64_t>(req.FilterLifetime_max))) {//修改filter的raw_data值
    return false;
  }
  ROS_INFO("FIRST BULKREAD");
  ars_40X_can_->send_radar_data(can_messages::FilterCfg);//传入带有帧头的数据，即在上一句raw_data值已更新得值
